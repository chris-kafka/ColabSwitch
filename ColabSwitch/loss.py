...
from af.src.misc import get_plddt, get_pae, get_fape_loss, get_dgram_loss, get_rmsd_loss_w, get_sc_rmsd, _np_rmsd
...
class _af_loss:
  def _get_fn(self):
    ...
    def mod(params, model_params, inputs, inputs_s=None, key, opt): #, inputs1 ,inputs2 ,
      ...
      if self.protocol == "switch":
        # concatenate target and binder sequence
        seq_pseudo_raw = jnp.concatenate([seq_pseudo] * self._copies, 1)
      ...
      update_seq(seq_pseudo_raw, inputs_s)
      ...
      outputs_sw = self._runner.apply(model_params, key, inputs_s)
      ...
      if self.protocol in ["binder",'switch']:
        losses["plddt"] = plddt_loss[...,self._target_len:].mean()
      else:
        losses["plddt"] = plddt_loss.mean()
      if self.protocol == "switch":
        loss_rmsd = 1/ _np_rmsd(outputs_s["structure_module"]["final_atom_positions"][:,1,:], outputs["structure_module"]["final_atom_positions"][:self.target_len,1,:], copies=self._copies)
